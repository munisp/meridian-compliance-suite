...
def test_03_outage_retries_no_loss_then_dlq():
    ...
    assert client.get(f"/v1/str/{rec['id']}").status_code == 200
    ...
    assert client.get("/v1/str/dlq/depth").json()["dlq_depth"][TENANT] == 1
...
def test_04_requeue_rbac_denied_for_auditor_and_anonymous():
    ...
    for _ in range(3):
        worker.process_due()
    assert client.get(f"/v1/str/{rec['id']}").json()["status"] == "dlq"
...
def test_10_audit_trail_forensic_fields_and_hash_chain():
    ...
    created = next(r for r in records if r["new_status"] == "pending")
    ...
    assert created["actor"] == "kyc-engine"
    ...
    fid = client.post("/v1/str", json={
        ...
        "subject_ref": "c", "payload": {}, "actor": "t"}).json()["id"]
    worker.process_due()
    with open(path) as fh:
